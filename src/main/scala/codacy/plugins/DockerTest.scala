...
object DockerTest {
  ...
  def main(args: Array[String]) {
    ...
    val patternName = args.drop(2).headOption
    ...
    typeOfTests.collect { case typeOfTest if possibleTestNames.contains(typeOfTest) =>
      dockerImageName.map { dockerName =>
        ...
        val result = possibleTests
          .map(test => run(plugin, testSources, test, typeOfTest, dockerName, patternName))
          .forall(identity)
  ...
  private def run(plugin: DockerPlugin, testSources: Seq[Path], test: ITest, testRequest: String, dockerImageName: String, patternName: Option[String]): Boolean = {
    if ((allTestKey :+ test.opt).contains(testRequest)) {
      test.run(plugin, testSources, dockerImageName, patternName) match {
        case true =>
